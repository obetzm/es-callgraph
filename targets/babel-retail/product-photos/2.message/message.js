...
var util = {
  serverError: function serverError(method, err) {
    console.log("".concat(constants.MODULE, " ").concat(method, " - ").concat(constants.ERROR_SERVER, ": ").concat(err));
  ...
  decrypt: function decrypt(field, value) {
    return kms.decrypt({
      ...
      return BbPromise.reject(field, error);
    });
  }
  /**
...
var impl = {
  ...
  ensureAuthTokenDecrypted: function ensureAuthTokenDecrypted(event) {
    if (!twilio.sdk) {
      ...
      }).catch(function (field, error) {
        return BbPromise.reject(new ServerError("".concat(constants.METHOD_ENSURE_TWILIO_INITIALIZED, " - Error decrypting '").concat(field, "': ").concat(error)));
  ...
  sendMessage: function sendMessage(event) {
    return twilio.messagesCreate({
      ...
      body: ["Hello ".concat(event.photographer.name, "!"), 'Please snap a pic of:', "  ".concat(event.data.name), 'Created by:', "  ".concat(event.merchantName)].join('\n').trim()
    }).catch(function (err) {
      return BbPromise.reject(new ServerError("".concat(constants.METHOD_SEND_MESSAGE, " - Error sending message to photographer via Twilio: ").concat(JSON.stringify(err, null, 2))));
    } // eslint-disable-line comma-dangle
    );
  } // Example event:
  // {
...
module.exports = {
  handler: function handler(event, context, callback) {
    ...
    impl.ensureAuthTokenDecrypted(event).then(impl.sendMessage).then(function (message) {
      console.log("Success: ".concat(JSON.stringify(message, null, 2)));
      callback(null, event);
    }).catch(function (ex) {
      console.log(JSON.stringify(ex, null, 2));
      callback("".concat(constants.MODULE, " ").concat(ex.message, ":\n").concat(ex.stack));
    });
  }
};