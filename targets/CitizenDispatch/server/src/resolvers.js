...
exports.getSystemInformation =  (queryFields = {}) => {
  const response = {}
  if ('requestEpicenter' in queryFields) {
    const requests = exports.getRequests();
  ...
  return dynamoDbLib.getOne('requests', { 'id': id })
    .then(record => {
      ...
      if ('team' in queryFields) {
        if (record.teamId) {
          newRecord.team = exports.getTeam({ id: record.teamId }, queryFields.team)
        } else {
          newRecord.team = null;
        }
      }
      ...
      if ('user' in queryFields) {
        if (record.userId) {
          newRecord.user = exports.getDispatch({ id: record.userId }, queryFields.user)
        } else {
          newRecord.user = null;
        }
      }
...
exports.getUserLocationHistory = ({ id }) => {
  ...
  return dynamoDbLib.query(params)
...
exports.upsertUser = ({ id, fields }) => {
  ...
  return dynamoDbLib.update(params)
  ...
  .then(records => {
    ...
    if ('leader' in queryFields) {
      newRecords = newRecords.map(record => {
        if (record.leaderId) {
          const leader = exports.getUser({ id: record.leaderId }, queryFields.leader)
